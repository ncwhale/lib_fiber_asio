...
class promise_executor {
 public:
  explicit promise_executor(const std::shared_ptr<boost::fibers::promise<T>>& p) : p_(p) {}
...
#define BOOST_ASIO_PRIVATE_PROMISE_SELECTOR_DEF(n)                        \
  ...
  class promise_handler_selector<void(Arg, BOOST_ASIO_VARIADIC_TARGS(n))> \
...
class promise_handler : public promise_handler_selector<Signature> {
 public:
  ...
  promise_handler(use_fiber_future_t<Allocator> u)
...
inline void asio_handler_invoke(
    Function& f, packaged_handler<Function1, Allocator, Result>* h) {
...
template <typename Function, typename Function1, typename Allocator,
          typename Result>
inline void asio_handler_invoke(
    const Function& f, packaged_handler<Function1, Allocator, Result>* h) {
  typename packaged_handler<Function1, Allocator, Result>::executor_type ex(
      h->get_executor());
  ex.dispatch(f, std::allocator<void>());
}
...
class async_result<fibers::use_fiber_future_t<Allocator>, Result(Args...)>
    : public fibers::detail::promise_async_result<void(typename decay<Args>::type...),
                                          Allocator> {
 public:
  explicit async_result(
      typename fibers::detail::promise_async_result<void(typename decay<Args>::type...),
                                            Allocator>::completion_handler_type&
          h)
      : fibers::detail::promise_async_result<void(typename decay<Args>::type...),
                                     Allocator>(h) {}
};
...
class async_result<fibers::use_fiber_future_t<Allocator>, Result()>
    : public fibers::detail::promise_async_result<void(), Allocator> {
 public:
  explicit async_result(typename fibers::detail::promise_async_result<
...
#define BOOST_ASIO_PRIVATE_ASYNC_RESULT_DEF(n)                             \
  ...
  class async_result<fibers::use_fiber_future_t<Allocator>,                        \
   ...
   public:                                                                 \
    explicit async_result(typename fibers::detail::promise_async_result<           \
        ...
        : fibers::detail::promise_async_result<void(BOOST_ASIO_VARIADIC_DECAY(n)), \
                                       Allocator>(h) {}                    \
  };                                                                       \
                                                                           \
  template <typename Function, typename Allocator, typename Result,        \
